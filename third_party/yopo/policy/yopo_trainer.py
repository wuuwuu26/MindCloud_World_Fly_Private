...
class YopoTrainer:
    ...
    def train_one_epoch(self, epoch: int, total_progress):
        ...
        traj_losses, score_losses, smooth_losses, safety_losses, goal_losses, acc_losses, height_losses, start_time = [], [], [], [], [], [], [], time.time()
        for step, (depth, pos, rot, obs_b, map_id) in enumerate(self.train_dataloader):  # obs: body frame
            ...
            trajectory_loss, score_loss, smooth_cost, safety_cost, goal_cost, acc_cost, height_cost = self.forward_and_compute_loss(depth, pos, rot, obs_b, map_id)
            ...
            height_losses.append(self.loss_weight[0] * height_cost.item())
            ...
            if step % inspect_interval == inspect_interval - 1:
                ...
                self.tensorboard_log.add_scalar("Detail/HeightLoss", np.mean(height_losses), epoch * len(self.train_dataloader) + step)
                traj_losses, score_losses, smooth_losses, safety_losses, goal_losses, acc_losses, height_losses, start_time = [], [], [], [], [], [], [], time.time()
    ...
    def forward_and_compute_loss(self, depth, pos, rot, obs_b, map_id):
        ...
        end_pos_w[:, 2] = fixed_height
        # [B*V*H, 3, 3]: [px, py, pz; vx, vy, vz; ax, ay, az]
        end_state_w = torch.stack([end_pos_w, end_vel_w, end_acc_w], dim=1)

        smooth_cost, safety_cost, goal_cost, acc_cost, height_cost = self.yopo_loss(start_state_w, end_state_w, goal_w, map_id)
        trajectory_loss = (smooth_cost + safety_cost + goal_cost + acc_cost + height_cost).mean()

        score_label = (smooth_cost + safety_cost + goal_cost + acc_cost + height_cost).clone().detach()
        score_loss = F.smooth_l1_loss(score_flat, score_label)
        return trajectory_loss, score_loss, smooth_cost.mean(), safety_cost.mean(), goal_cost.mean(), acc_cost.mean(), height_cost.mean()
