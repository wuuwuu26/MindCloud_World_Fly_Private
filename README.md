...
- **终点接管（< 12m）**：两端各有一层。服务端 `FINAL_APPROACH_DIST=12.0` 内停止网络推理，直接规划一条"末端速度/加速度为零"的五阶多项式——近目标处 `argmin(score)` 会反复挑出过冲/折返轨迹，叠加 `plan_from_reference` 会让目标方向观测翻转、位置速度持续振荡而永远到不了。客户端同样在 `yopoFinalApproachDist=12.0`（按**3D 距离**判定，水平近但垂直远时不切换）内由 PD 接管收敛（位置环 + 速度/加速度前馈，带速度变化率上限避免抖动）。
  - **接管区内射线避障仍生效且优先**：探测与刹车全程照跑；一旦射线层判定有真实威胁（滤波后 `brake < 0.97`），切向绕行（tan）按迟滞 + 0.4 s 淡入加入，并与 PD 走同一套"侧向速度预算"（基准 ≥ `yopoCruiseMinSpd`），避障**优先于** PD 的直冲分量（`rep`/`vGo`/`vRep` 仍保持关闭，避免与 PD 沿目标线来回对抗）；走廊恢复畅通（`brake > 0.995`）后退回纯 PD。**接管末段射线完全让步**：进入最后 `yopoTakeoverSteerEndDist = 3.0` m，或目标贴墙（`gateBeyondGoal` 放行、`vCloseMax = ∞`，即威胁就在目标处）时，射线层立刻收手——`brake` 强制置 1（不再缩放速度目标）、切向 steer 关闭、减速前馈撤掉，把控制权彻底交还 PD。否则射线把贴在墙上的目标判成障碍，tan 反复把无人机推离墙、PD 又把其拉回目标，二者拉锯 → `yopoArrived` 始终因速度被持续推离而判不出 → 末端一直晃。该让步只发生在已到位/快到位的末端，PD 的 `holdMaxV = √(2ad)` 已保证物理可停，射线在此处只有抖动、没有收益。
...
- **upPush + vSafeDown（地面与下降安全）**：`upPush` 在 `groundGap < yopoMinAlt`（头顶/脚下净空不足）时 `= (yopoMinAlt − groundGap)·yopoAvoidGain·0.5`（下降净空不足时用 0.6 系数取较大者）向上推；头顶净空不足时用 `vSafeUp = √(2·aDecel·(vUpDist − standoff))` 限制上推，避免撞顶。`vSafeDown` 取正下方净空 `downGap = min(groundGap, vDownDist)`：若 `≤ standoff` 则禁止下降（=0），否则 `vSafeDown = √(2·aDecel·(downGap − standoff))`，调用点把它作为"下降速度硬上限"直接夹住 `velTargetY`。竖向威胁不写入 `dAhead`（无水平方向），故不会误刹前向巡航——地面/天花板安全完全由这两项独立处理。
...
| `yopoAvoidStop` | 6.0 | **竖直**安全净距 (m)：驱动上/下净空刹车（vSafeUp/vSafeDown）与竖直越障封锁距离；刻意不随 StopH 上调，否则净空小于它时完全禁止下降（`vSafeDown=0`），低目标/近地无法下降 |
| `yopoMinAlt` | 3.0 | 最小离地/离顶净空 (m)：低于它触发向上推离（2.5 → 3.0，贴地飞行更少） |
