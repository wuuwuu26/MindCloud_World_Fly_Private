...
[🎬 点击观看功能演示（连续避障 170MB + 脱困逃逸 70MB 高清）](https://wuuwuu26.github.io/MindCloud_World_Fly_Private/video.html)
...
1. 飞行模式下，按 **`T`** 进入目标选取模式：主视图切换为**俯视地图**（与放置模式类似的自由视角——左键拖动平移、中键拖动倾转、滚轮缩放），左下角 YOPO 菜单与右下角 **TARGET ALTITUDE** 高度面板同时出现。
2. **鼠标左键点击地图任意位置**放置目标点（可反复点击调整）；高度用右下面板直接键入或滚轮调节（±1 m，Shift ±5 m）。
