...
- **终点接管（< 12m）**：两端各有一层。服务端 `FINAL_APPROACH_DIST=12.0` 内停止网络推理，直接规划一条"末端速度/加速度为零"的五阶多项式——近目标处 `argmin(score)` 会反复挑出过冲/折返轨迹，叠加 `plan_from_reference` 会让目标方向观测翻转、位置速度持续振荡而永远到不了。客户端同样在 `yopoFinalApproachDist=12.0`（按**3D 距离**判定，水平近但垂直远时不切换）内由 PD 接管收敛（位置环 + 速度/加速度前馈，带速度变化率上限避免抖动）。
  - **接管区内射线避障仍生效且优先**：探测与刹车全程照跑；一旦射线层判定有真实威胁（滤波后 `brake < 0.97`），切向绕行（tan）按迟滞 + 0.4 s 淡入加入，并与 PD 走同一套"侧向速度预算"（基准 ≥ `yopoCruiseMinSpd`），避障**优先于** PD 的直冲分量（`rep`/`vGo`/`vRep` 仍保持关闭，避免与 PD 沿目标线来回对抗）；走廊恢复畅通（`brake > 0.995`）后退回纯 PD。**接管末段射线完全让步**：进入最后 `yopoTakeoverSteerEndDist = 3.0` m，或目标贴墙（`gateBeyondGoal` 放行、`vCloseMax = ∞`，即威胁就在目标处）时，射线层立刻收手——`brake` 强制置 1（不再缩放速度目标）、切向 steer 关闭、减速前馈撤掉，把控制权彻底交还 PD。否则射线把贴在墙上的目标判成障碍，tan 反复把无人机推离墙、PD 又把其拉回目标，二者拉锯 → `yopoArrived` 始终因速度被持续推离而判不出 → 末端一直晃。该让步只关闭**转向 steer**（tan/vGo/vRep/upPush），射线层不再反复推搡，但**仍保留 `brake` 与"剔除推离目标分量"后的 rep**：贴墙目标不会被 rep 推出去 swing，而侧/后方障碍仍被稳定推开（解决"后方也容易撞到障碍物"）；PD 的 `holdMaxV = √(2ad)` 已保证物理可停。更进一步，整个射线避障块此前在 `arrived` 之后被 `!yopoArrived` 整段跳过 → 到达后完全无避障、侧/后方来障会撞；现已让到达后**继续运行射线避障**（仅 brake + 清洗后 rep，绝不做 steer），即"射线避障时刻优先"：停在贴墙/障碍旁的目标点时，侧后方障碍仍把它推离到安全距离，而不是钉在危险点。
